...
-- This script creates all tables, indexes, RLS policies, storage buckets, and functions
-- for the current model of the job platform PWA
...
DROP TABLE IF EXISTS round_tokens CASCADE;
...
DELETE FROM storage.buckets WHERE id IN ('applications', 'videos', 'avatars');
...
-- Create jobbie_transactions table
CREATE TABLE jobbie_transactions (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE INDEX idx_profiles_jobbies ON profiles(jobbies);
...
-- Jobbie transactions indexes
CREATE INDEX idx_jobbie_transactions_user_id ON jobbie_transactions(user_id);
CREATE INDEX idx_jobbie_transactions_created_at ON jobbie_transactions(created_at DESC);
CREATE INDEX idx_jobbie_transactions_type ON jobbie_transactions(type);
CREATE INDEX idx_jobbie_transactions_reference ON jobbie_transactions(reference_id, reference_type);

-- Round tokens indexes
CREATE INDEX idx_round_tokens_user_id ON round_tokens(user_id);
CREATE INDEX idx_round_tokens_job_id ON round_tokens(job_id);
CREATE INDEX idx_round_tokens_round_number ON round_tokens(round_number);
CREATE INDEX idx_round_tokens_status ON round_tokens(status);
CREATE INDEX idx_round_tokens_job_round ON round_tokens(job_id, round_number);
...
-- RLS Policies for jobbie_transactions
CREATE POLICY "Users can view own transactions" ON jobbie_transactions FOR SELECT USING (
  auth.uid() = user_id
);
CREATE POLICY "Users can insert own transactions" ON jobbie_transactions FOR INSERT WITH CHECK (
  auth.uid() = user_id
);

-- RLS Policies for round_tokens
...
-- Function to update user jobbies
...
-- Create trigger for jobbie updates
CREATE TRIGGER update_user_jobbies_trigger
    AFTER INSERT ON jobbie_transactions
    FOR EACH ROW EXECUTE FUNCTION update_user_jobbies();
...
SELECT 
    ...
    SUM(CASE WHEN jt.type = 'earn' THEN jt.amount ELSE 0 END) as total_jobbies_earned,
    SUM(CASE WHEN jt.type = 'spend' THEN jt.amount ELSE 0 END) as total_jobbies_spent
FROM profiles p
...
BEGIN
    ...
    RAISE NOTICE 'Tables created: profiles, jobs, applications, jobbie_transactions, round_tokens, job_views, saved_jobs, notifications, referrals, assessments, user_assessments';
    ...
    RAISE NOTICE 'Views created: job_stats, user_stats';
END $$; 